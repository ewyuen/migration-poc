...
Define OrchestratorV3, a LangGraph-based reimplementation of the migration orchestrator that models the 6-stage migration workflow as an explicit `StateGraph` (nodes, edges, and a typed state schema) while wrapping the existing agents unchanged and preserving the OrchestratorV2 entry point and audit trail behavior.
