...
Define the deterministic (non-LLM) skeleton generation stage: parsing Gherkin `.feature` content, extracting steps, inferring Cucumber Expression parameter types, and emitting a valid, compilable `StepDefinitions.cs` skeleton with `[Binding]` class and `// TODO` method stubs.
